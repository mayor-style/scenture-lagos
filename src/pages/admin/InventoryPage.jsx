...
import { Search, FileText, Download, RefreshCw, Package, CheckCircle, AlertTriangle, Plus, Minus, ChevronLeft, ChevronRight, Settings, History } from 'lucide-react'; // Added History icon
...
const InventoryPage = () => {
    ...
    const handleAdjustment = async () => {
        ...
        // Client-side validation for negative stock (backend also validates)
        if (selectedItem.stockQuantity + adjustmentQuantity < 0) {
            addToast('Adjustment would result in negative stock. Please enter a valid quantity.', 'error');
            return;
        }

        setAdjustmentLoading(true);
        try {
            // Pass allowNegative: false explicitly if you want the backend to enforce non-negative stock.
    ...
    const renderItemCard = (item) => (
        <Card key={item._id} className="group border-primary/20 bg-background hover:shadow-lg transition-all duration-300">
            <CardContent className="p-4 flex flex-col justify-between h-full">
                ...
                {/* Action Buttons: Adjust Stock and View History */}
                <div className="flex flex-col gap-2 mt-4">
                    <Button variant="outline" size="sm" onClick={() => openAdjustmentModal(item)} disabled={loading || adjustmentLoading} className="group w-full hover:bg-primary/10">
                        <Package className="mr-2 h-4 w-4 group-hover:text-primary" />
                        Adjust Stock
                    </Button>
                    {/* NEW: Button to view stock history */}
                 <Button asChild variant="ghost" size="sm" className="group w-full text-muted-foreground hover:bg-muted hover:text-primary">
                <Link to={`/admin/inventory/${item._id}/history`} className='flex'>

...
import { Trash2, ShoppingBag, ArrowRight, Plus, Minus, AlertTriangle } from 'lucide-react';
...
// Memoized Cart Item Component - Now with validation feedback
const CartItem = React.memo(({ item, onUpdate, onRemove }) => {
  // Disable the 'add' button if cart quantity meets or exceeds available stock
      ...
      <div className="flex-grow flex flex-col gap-4">
        <div className="flex justify-between items-start">
          ...
          <div className="text-right">
            <p className={`font-semibold text-lg text-slate-900 ${item.hasPriceChanged ? 'text-orange-600' : ''}`}>
              {formatPrice(item.price)}
            </p>
            {item.hasPriceChanged && (
              <p className="text-xs text-slate-500 line-through">{formatPrice(item.oldPrice)}</p>
            )}
          </div>
        </div>
        ...
        {item.hasPriceChanged && (
          <div className="flex items-center gap-2 text-xs text-orange-600 bg-orange-50 p-2 rounded-lg">
            ...
            <span>Price has been updated.</span>
          </div>
        )}
        {item.quantity > item.availableStock && (
           <div className="flex items-center gap-2 text-xs text-red-600 bg-red-50 p-2 rounded-lg">
            <AlertTriangle size={14} />
            <span>Quantity adjusted to available stock.</span>
          </div>
        )}
        
        <div className="flex justify-between items-center">
          <div className="flex items-center border border-slate-200 rounded-full">
            <button onClick={() => onUpdate({ itemId: item._id, quantity: item.quantity - 1 })} disabled={item.quantity <= 1} className="p-2 text-slate-500 disabled:text-slate-300 hover:bg-slate-100 rounded-l-full transition-colors"><Minus size={16} /></button>
            <span className="w-10 text-center text-sm font-medium text-slate-800">{item.quantity}</span>
            <button onClick={() => onUpdate({ itemId: item._id, quantity: item.quantity + 1 })} disabled={isStockMaxed} className="p-2 text-slate-500 disabled:text-slate-300 disabled:cursor-not-allowed hover:bg-slate-100 rounded-r-full transition-colors"><Plus size={16} /></button>
          </div>
...
// Memoized Order Summary Component (No changes needed)
...
// Empty Cart Component (No changes needed)
const EmptyCart = () => (
...
// Cart Page Loader (No changes needed)
...
const CartPage = () => {
  ...
  // Handler for updating quantity that prevents quantities below 1
    ...
    <div className="bg-slate-50 min-h-screen">
      <div className="container max-w-7xl px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-10 lg:mb-12">
          ...
          <p className="text-slate-500 mt-2 text-lg">You have {cart.totalItems} {cart.totalItems === 1 ? 'item' : 'items'} in your cart.</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 lg:gap-12">
          <div className="xl:col-span-2 bg-white rounded-3xl shadow-sm border border-slate-100 divide-y divide-slate-100">
             <AnimatePresence>
               {cart.items.map((item) => (
                  <CartItem key={item._id} item={item} onUpdate={handleUpdate} onRemove={removeFromCart} />
               ))}
            </AnimatePresence>
          </div>
